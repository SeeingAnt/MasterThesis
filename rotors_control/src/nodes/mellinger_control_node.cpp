...
MellingerControlNode::MellingerControlNode() {
    ...
    init_sub_ = nh.advertiseService("init_model",&MellingerControlNode::InitService,this);

    gains_sub_ = nh.subscribe("crazyflie2/ControlGains", 1, &MellingerControlNode::CallbackGainsControl,this);
...
void MellingerControlNode::CallbackGainsControl(const rotors_comm::GainMSG &control_gains){

     mellinger_controller_.SetGainP(control_gains.Gain_p.data());
     mellinger_controller_.SetGainD(control_gains.Gain_d.data());
...
bool MellingerControlNode::InitService(rotors_comm::init_service::Request  &req,
                                            rotors_comm::init_service::Response &res){

  InitializeParams();
  if (mellinger_controller_.hover_is_active == true)
    mellinger_controller_.setHover();
  if (mellinger_controller_.path_is_active == true)
