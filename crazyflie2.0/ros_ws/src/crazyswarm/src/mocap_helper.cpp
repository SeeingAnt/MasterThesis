...
int main(int argc, char **argv)
{
  ...
  ros::Publisher pose_pub = nl.advertise<crazyswarm::pose>("cf_pose", 1000);
  crazyswarm::pose feed_pose; 
    ...
    if (mocap->supportsObjectTracking()) {
      ...
      for (auto const& object: objects) {
        ...
        if (object.occluded() == false) {
          ...
          feed_pose.x = position(0);
          feed_pose.y = position(1);
          feed_pose.z = position(2);

          feed_pose.qx = rotation.vec()(0);
          feed_pose.qy = rotation.vec()(1);
          feed_pose.qz = rotation.vec()(2);
          feed_pose.qw = rotation.w();
          pose_pub.publish(feed_pose);
        }
      }
    }
