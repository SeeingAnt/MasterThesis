...
from crazyflie_driver.msg import TrajectoryPolynomialPiece, FullState, Gains
...
class Crazyflie:
    def __init__(self, id, initialPosition, tf):
        ...
        self.cmdGainsPublisher = rospy.Publisher(prefix + "/cmd_gains", Gains, queue_size=1)
        self.cmdGainsMsq = Gains()
        self.cmdGainsMsq.header.seq = 0
        self.cmdGainsMsq.header.frame_id = "/world"
    ...
    def cmdFullState(self, pos, vel, acc, yaw, omega, pitch=0.0, roll=0.0):
        self.cmdFullStateMsg.header.stamp = rospy.Time.now()
        self.cmdFullStateMsg.header.seq += 1
        self.cmdFullStateMsg.pose.position.x    = pos[0]
        self.cmdFullStateMsg.pose.position.y    = pos[1]
        ...
        self.cmdFullStateMsg.acc.y              = acc[1]
        self.cmdFullStateMsg.acc.z              = acc[2]
        self.cmdFullStateMsg.pose.orientation   = geometry_msgs.msg.Quaternion(*tf_conversions.transformations.quaternion_from_euler(roll, pitch, yaw))
        self.cmdFullStateMsg.twist.angular.x    = omega[0]
        self.cmdFullStateMsg.twist.angular.y    = omega[1]
        self.cmdFullStateMsg.twist.angular.z    = omega[2]
        self.cmdFullStatePublisher.publish(self.cmdFullStateMsg)

    def cmdGains(self, gain, port):
        self.cmdGainsMsq.header.stamp = rospy.Time.now()
        self.cmdGainsMsq.header.seq += 1
        self.cmdGainsMsq.gain1    = gain[0]
        self.cmdGainsMsq.gain2    = gain[1]
        self.cmdGainsMsq.gain3    = gain[2]
        self.cmdGainsMsq.gain4    = gain[3]
        self.cmdGainsMsq.gain5    = gain[4]
        self.cmdGainsMsq.gain6    = gain[5]
        self.cmdGainsMsq.gain7    = gain[6]
        self.cmdGainsMsq.gain8    = gain[7]
        self.cmdGainsMsq.gain9    = gain[8]
        self.cmdGainsMsq.gain10   = gain[9]
        self.cmdGainsMsq.gain11  = gain[10]
        self.cmdGainsMsq.gain12  = gain[11]
        self.cmdGainsMsq.port    = port
        self.cmdGainsPublisher.publish(self.cmdGainsMsq)
